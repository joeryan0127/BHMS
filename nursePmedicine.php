...
// echo '<script>window.location="nurseConsultation.php";</script>';
...
<?php
                    ...
                    <!-- <select  name ="Medicine[]" class="form-control selectpicker"  data-live-search="true"   >
                     -->
                    <input type = "text" name="Medicine[]" list="browN_id" class="form-control" placeholder ="Select Medicine"  required >
            <datalist id="browN_id">
...
$(document).ready(function(){
    var html=' <tr><td><?php $statement =$conn->prepare("SELECT * from medicine WHERE Expiry_date >= :today AND remark=''"); $statement->bindValue(':today',$date); $statement->execute(); $meds=$statement->fetchAll()?> <input type = "text" name="Medicine[]" list="browN_id" class="form-control" placeholder ="Select Medicine"  required ><datalist id="browN_id"><?php foreach ($meds as $row1): ?>
                <option value=<?= $row1['Med_id'] ?> ><?= $row1['Med_name'] ?> <?= $row1['Med_dosage'] ?> <?= $row1['Med_quantity'] ?> </option> <?php endforeach ?></datalist></td> <td><input type ="text" class = "form-control" name ="quantity[]" placeholder ="Quantity"  required></td><td><input type ="button" class = "btn btn-outline-danger" name ="Remove" value="x" id="Del"></td></tr>';
var x=1;
