...
if($checkpwd===false){
    header("location: ../admindash.php?error=WrongCurrentpass");
    echo $checkpwd;
}else if($new1!==$new2){

    header("location: ../admindash.php?error=Wrongpass");
    ...
    if(!ctype_upper($new1)&& !ctype_lower($new1)){
     ...
     header("Location: ../admindash.php?error=notlowercaase");
     exit();
    }
 
    }else{
     header("Location: ../admindash.php?error=not8");
     exit();
